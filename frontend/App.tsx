...
export default function App() {
  ...
  useEffect(() => {
    // Bootstrap auth first so the app can immediately decide whether to show auth or the dashboard.
    ...
    const handleDeepLink = (url: string | null) => {
      ...
      // Deep links must also work after a cold start, not only when the app is already open.
  ...
  const renderAuthScreen = () => {
    switch (authMode) {
      case 'login':
        return (
          <LoginScreen
            onOpenForgotPassword={() => setAuthMode('forgot-password')}
  ...
  const renderScreen = () => {
    // Keep tab routing explicit so each main area remains easy to trace and maintain.
    switch (currentTab) {
      case 'Home':
        return <HomeScreen />;
      case 'Schede':
      ...
      <QueryClientProvider client={queryClient}>
        <View className="flex-1 bg-[#0A0A0A]">
          {session && authMode !== 'reset-password' ? (
            <View className="flex-1 bg-[#0A0A0A]">
              // The active workout overlay stays mounted globally so it can interrupt any screen.
              {renderScreen()}
              {/* TAB BAR ... */}
                ...
                <>
                  ...
                  <View className="absolute bottom-8 left-6 right-6 h-20 bg-black/90 rounded-[35px] flex-row items-center justify-around px-4 shadow-2xl border border-white/5 backdrop-blur-md">
                    <TouchableOpacity onPress={() => setTab('Home')} className="items-center p-2">
                      <Home
                        size={24}
                        color={currentTab === 'Home' ? '#10B981' : '#FFF'}
                        strokeWidth={currentTab === 'Home' ? 3 : 2}
                      />
                      {currentTab === 'Home' && (
                        <View className="w-1 h-1 rounded-full bg-[#10B981] mt-1.5" />
                      )}
                    </TouchableOpacity>
                    ...
                    <TouchableOpacity onPress={() => setTab('Schede')} className="items-center p-2">
                      <Layout
                        size={24}
                        color={currentTab === 'Schede' ? '#10B981' : '#FFF'}
                        strokeWidth={currentTab === 'Schede' ? 3 : 2}
                      />
                      {currentTab === 'Schede' && (
                        <View className="w-1 h-1 rounded-full bg-[#10B981] mt-1.5" />
                      )}
                    ...
                    >
                      <History
                        size={24}
                        color={currentTab === 'History' ? '#10B981' : '#FFF'}
                        strokeWidth={currentTab === 'History' ? 3 : 2}
                      />
                      {currentTab === 'History' && (
                        <View className="w-1 h-1 rounded-full bg-[#10B981] mt-1.5" />
                      )}
                    ...
                    >
                      <User
                        size={24}
                        color={currentTab === 'Profile' ? '#10B981' : '#FFF'}
                        strokeWidth={currentTab === 'Profile' ? 3 : 2}
                      />
                      {currentTab === 'Profile' && (
                        <View className="w-1 h-1 rounded-full bg-[#10B981] mt-1.5" />
                      )}
