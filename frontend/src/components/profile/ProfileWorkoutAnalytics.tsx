...
export default function ProfileWorkoutAnalytics({
  ...
  // Derive monthly and chart-ready stats once so the render path stays lightweight.
  const analytics = useMemo(() => {
    // Work on a sorted copy so the original sessions array remains untouched.
    ...
    const calcTotals = (items: ProfileSession[]) => {
      // Compute totals with a single pass per metric to keep the component deterministic and cheap.
    ...
    const durationDelta =
      lastMonth.durationSeconds > 0
        ? ((thisMonth.durationSeconds - lastMonth.durationSeconds) / lastMonth.durationSeconds) *
          100
    ...
    <View className="mb-10">
      ...
      <View className="bg-black/40 rounded-[32px] border border-white/5 p-4 mb-4">
        <View className="flex-row items-center justify-between mb-3 px-1">
          ...
          <Text className="text-white/80 font-bold text-[10px] uppercase tracking-[1px]">
            {compareLabel}
          </Text>
        </View>
      ...
      <View className="bg-black/40 rounded-[32px] border border-white/5 p-4">
        <View className="flex-row items-center justify-between mb-4 px-1">
          ...
          <View
            className={`px-2.5 py-1 rounded-full border ${analytics.durationDelta >= 0 ? 'bg-emerald-500/15 border-emerald-500/20' : 'bg-red-500/15 border-red-500/20'}`}
          >
            <Text
              className={`font-black text-[10px] uppercase tracking-[1px] ${analytics.durationDelta >= 0 ? 'text-emerald-300' : 'text-red-300'}`}
            >
              {analytics.durationDelta >= 0 ? '+' : ''}
              {Math.round(analytics.durationDelta)}%
            </Text>
          </View>
        </View>
        ...
        <View className="flex-row flex-wrap gap-3">
          <View className="flex-1 min-w-[46%] bg-white/5 rounded-2xl p-4 border border-white/5">
            <Text className="text-white/70 text-[10px] uppercase tracking-[1px] mb-1">
              {thisMonthLabel}
            </Text>
            <Text className="text-white font-black text-2xl">
              {analytics.thisMonth.sessionsCount}
            </Text>
          ...
          <View className="flex-1 min-w-[46%] bg-white/5 rounded-2xl p-4 border border-white/5">
            ...
            <Text className="text-white font-black text-2xl">
              {analytics.lastMonth.sessionsCount}
            </Text>
            <Text className="text-white/80 text-[11px] mt-1">{sessionsLabel}</Text>
            <Text className="text-white/80 text-[11px] mt-1">
