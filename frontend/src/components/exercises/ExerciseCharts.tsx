...
export default function ExerciseCharts({ historyData }: ExerciseChartsProps) {
  ...
  if (activeMetric === '1RM') {
     chartData = sortedData.map((session: any) => {
        const sessionMaxWeight = Math.max(...session.sets.map((set: any) => parseFloat(set.weight) || 0));
        return {
           value: sessionMaxWeight,
           label: new Date(session.completed_at).getDate() + '/' + (new Date(session.completed_at).getMonth() + 1),
           frontColor: '#10B981',
        };
     });
     ...
     chartData = sortedData.map((session: any) => {
        const totalReps = session.sets.reduce((acc: number, set: any) => acc + (parseInt(set.reps, 10) || 0), 0);
        return {
           value: totalReps,
           label: new Date(session.completed_at).getDate() + '/' + (new Date(session.completed_at).getMonth() + 1),
           frontColor: '#F59E0B',
  ...
  const RenderTab = ({ metric, label }: { metric: ChartType, label: string }) => {
    ...
    return (
      ...
      >
        <Text className={`font-black text-[10px] uppercase tracking-widest ${isActive ? 'text-white' : 'text-gray-500'}`}>
          {label}
        </Text>
      </TouchableOpacity>
    );
        ...
        {activeMetric === '1RM' ? (
          <LineChart
            ...
            pointerConfig={{
              ...
              radius: 4,
              pointerColor: '#FFF',
              pointerComponent: () => <View className="w-3 h-3 rounded-full bg-white border-[3px] border-[#10B981]" />
            }}
          />
        ) : (
          <BarChart
