...
- [x] **Pulizia tabelle legacy:** rinominare `workouts` e `sets` in `*_legacy_20260326`, testare app 2-3 giorni, poi drop definitivo solo se zero regressioni.
- [ ] **Audit dipendenze frontend:** valutare `npm audit` e applicare fix non-breaking prima della release.
- [ ] **DowngradeGateModal (Sistema di Gating Premium 📉):** Implementare un Modale UI che intercetti il downgrade di un utente da Premium a Free. Se possiede più di 4 schede, bloccare l'interfaccia e obbligarlo a sceglierne esattamente 4 da "Salvare", eliminando le altre per ripristinare i permessi del piano base.
